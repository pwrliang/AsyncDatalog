...
public class WorkerAddrMap implements Serializable {
    ...
    public void add(InetAddress machineAddr, int workerId) {
        machineAddrs[workerId] = machineAddr;
        L.info("Adding " + machineAddr + " at index:" + workerId);
        lastUnusedIndex++;
    }
    ...
    public int myIndex() {
        if (myIdx == -1) {
            ...
            try {
                addr = InetAddress.getByName(myAddrStr);
